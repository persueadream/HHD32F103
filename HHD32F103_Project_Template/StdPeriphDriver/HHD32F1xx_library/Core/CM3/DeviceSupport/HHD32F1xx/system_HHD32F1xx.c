...
#define SYSOSCCTRL_Val        0x00000002 //¾§ÕñÆµÂÊ·¶Î§£º2.5-16MHz
#define SYSPLLCLKSEL_Val      0x00000001	//external OSC
#define SYSPLL_SETUP          1
#define SYSPLLCTRL_Val        0x00004049//72MHz 8M HSE
//#define SYSPLLCTRL_Val        0x00004044 // 100MHz  25M XTALIN
//#define SYSPLLCTRL_Val        0x00004043 // 75MHz  25M XTALIN
//#define SYSPLLCTRL_Val        0x00004149   // 72MHZ  IN OSC  40M XTALIN
//#define SYSPLLCTRL_Val        0x00004102  // 72MHZ  IN OSC  40M XTALIN
...
#define __WDT_OSC_CLK        (32768UL)    /* Internal RC oscillator frequency */
    ...
    /* sys_pllclkin calculation */
      ...
      //#define  __SYS_PLLCLKOUT         (__SYS_PLLCLKIN * (SYSPLLCTRL_Val & 0x01F))
			#define  __SYS_PLLCLKOUT         (__SYS_PLLCLKIN * (SYSPLLCTRL_Val & 0x01F)/((SYSPLLCTRL_Val >> 6) & 0x03F)) //Modify By wangkui
    #else
      ...
      switch (HHD_SYSCON->SYSPLLCLKSEL & 0x03) {
          case 0:                       /* Internal RC oscillator             */
            //SystemCoreClock = __IRC_OSC_CLK * ((HHD_SYSCON->SYSPLLCTRL & 0x01F) + 1);
					  SystemCoreClock = __IRC_OSC_CLK * (HHD_SYSCON->SYSPLLCTRL & 0x01F);
            break;
          case 1:                       /* System oscillator                  */
            //SystemCoreClock = __SYS_OSC_CLK * ((HHD_SYSCON->SYSPLLCTRL & 0x01F) + 1);
					  SystemCoreClock = __SYS_OSC_CLK * (HHD_SYSCON->SYSPLLCTRL & 0x01F);
            break;
          case 2:                       /* Reserved                           */
          case 3:                       /* Reserved                           */
            SystemCoreClock = 0;
...
//  while (!(HHD_SYSCON->MAINCLKUEN & 0x01));     /* Wait Until Updated       */
  ...
  //HHD_SYSCON->SYSAPB1CLKCTRL = 0xFFFFFFFF;
	...
	//HHD_SYSCON->SYSAHBCLKCTRL |= 0x3<<2;	//Enable GPIOA/GPIOB
	//if ((HHD_GPIOB->PIN&0x00000004)==0x00000000)
	//	while(1);
}
