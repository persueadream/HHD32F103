...
int main(void)
{
	...
	// TIM5,TIM7 are basic timer, which only have one MR and no capture register
	// TIM5-14 are enhenced timer, which have 4 MR & 4 Capture register.
	/*                                                                --------*/
	
  /* ----------- Basic Timer 6 Periph clock enable, set TIM5 to run at 1MHz */
  TIM_Init(TIM4,1000000UL);
	...
	TIM_ConfigMatch(TIM4,9,TIM_MATCH_RESET_COUNTER);
